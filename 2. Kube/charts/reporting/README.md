...
```bash
helm install reporting ./2.\ Kube/charts/reporting -n app
```
...
helm upgrade reporting ./2.\ Kube/charts/reporting -n app \
  --set schedule="0 8 * * *"
```
...
```bash
helm install reporting ./2.\ Kube/charts/reporting -n staging \
  --set schedule="*/5 * * * *" \
